...
class DFRobot_GNSS(object):
  __m_flag   = 0x00                # mode flag
  __count    = 0x00                # acquisition count    
  __txbuf        = bytearray([0x00])          # i2c send buffer
  __gnss_all_data  = bytearray([0x00]*1300)     # gnss data
  __uart_i2c     =  0
...
class DFRobot_GNSS_I2C(DFRobot_GNSS): 
  def __init__(self, bus, addr):
    #print(addr)
    self.__addr = addr
    super(DFRobot_GNSS_I2C, self).__init__(bus,addr)

  def write_reg(self, reg, data):
    while 1:
      try:
        #print(self.__addr,reg,data)
